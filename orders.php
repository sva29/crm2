...
function logMessage($message) {
    global $pdo; // Доступ к глобальному объекту PDO
    
    // Абсолютный путь к файлу логов
    $logFile = realpath(__DIR__) . '/orders_log.txt';
...
if(isset($_POST['update_preview'])) {
    ...
    try {
        ...
        foreach($_SESSION['order_preview'] as &$item) {
            ...
            // Подготовленная проверка продукта
        ...
        logMessage("Database error during update: " . $e->getMessage());
        die("A database error has occurred.");
    } catch(Exception $e) {
        logMessage("General error during update: " . $e->getMessage());
        ...
        foreach($items as $index => $item) {
            if(!empty($item['item_id']) && !empty($item['quantity']) && $item['quantity'] > 0) {
                ...
                if($product) {
                    ...
                    if(!$stmt->fetch()) {
                        $warnings[] = "Продукт с ID {$product['product_id']} не существует в таблице products.";
                        continue;
                    }
                    ...
                    $stmt->execute([
                        ...
                        $product['product_id'],
                        $quantity,
                        isset($item['container_component_id']) ? $item['container_component_id'] : null // Тут добавили защиту
                    ]);
                    logMessage("Product item: product_id={$product['product_id']}, name={$product['product_name']}, quantity=$quantity");
                    
                    // Расчет сырья для продукта
                    $cost_data = calculateProductCost($pdo, $product['product_id'], $quantity, 'logMessage');
                    logMessage("Components for product_id={$product['product_id']}: " . json_encode($cost_data['components']));
                    ...
                    foreach($cost_data['components'] as $comp_id => $comp_data) {
                        if($comp_data['price_per_unit'] > 0) {
                            ...
                            logMessage("Added component_id=$comp_id for product_id={$product['product_id']}: quantity=$comp_quantity, price_per_unit={$comp_info['price_per_unit']}");
                        }
                    }
                    ...
                    if($component) {
                        ...
                        $container_component_id = isset($item['container_component_id']) ? $item['container_component_id'] : null; // Тут добавили защиту
                        $product_id = null;
